...
{% set data_page = 'social-doctrine' %}
{% include "partials/webdoor.html" %}
<main class="c-document c-document">
	<div class="c-document__holder">
		{% for item in data %}
			{% if (item.paragraph_partial != '') or (item.paragraph_section != '') or (item.paragraph_chapter != '') or (item.paragraph_title != '') %}
				<header class="c-document__header">
					...
					{% if item.paragraph_chapter != '' %}
						<h3 class="o-title o-title--3">{{ item.paragraph_chapter }}</h3>
					{% endif %}
					{% if item.paragraph_section != '' %}
						<h4 class="o-title o-title--4">{{ item.paragraph_section }}</h4>
					{% endif %}
					{% if item.paragraph_title != '' %}
						<h5 class="o-title o-title--5">{{ item.paragraph_title }}</h5>
					{% endif %}
				</header>
			{% endif %}
