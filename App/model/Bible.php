...
class Bible {
    ...
    public function getBible($book, $chapter){
        if (!$book) {
            ...
            $sql = 'select book_name, verser_number, chapter, verser_text from books inner join verser on books.id_book = verser.id_book where books.url_text = :url and chapter = 1 order by CAST(chapter AS unsigned), CAST(verser_number AS unsigned)';
