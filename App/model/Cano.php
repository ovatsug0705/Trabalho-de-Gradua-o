...
class Cano {
    ...
    public function getCano($cano){
        ...
        $sql = 'select paragraph_number, paragraph_text, paragraph_book, paragraph_partial, paragraph_title, paragraph_chapter, paragraph_article, ref_text, (select count(id_cano) from Canos) as count_paragraph from Canos left join Canos_references on Canos.id_cano = Canos_references.id_cano where paragraph_number > (:iId) and paragraph_number <= (:eId) order by CAST(paragraph_number AS unsigned)';
